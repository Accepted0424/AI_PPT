...
def call_api(book_path, prompt_file_path, temp=0.4, top=0.9):
    ...
    # temp_test = completion_0.model_dump_json()
    # print(temp_test)
    ...
    output_raw_first_path = os.path.join("output_raw", f"output_first_{i}.txt")
    with open(output_raw_first_path, 'w', encoding='utf-8') as f:
    ...
    3.每一章重点(###)知识点列举完之后用英语单词或短语概括该重点,用作关键词搜索图片,关键词应该与ppt的主题相符,用括号括起来,单独占一行,例如"(network),确保每一章下仅有一张图片"
    ...
    # temp_test = completion.model_dump_json()
    # print(temp_test)
    ...
    # 将生成的文本写入文件
    output_raw_second_path = os.path.join("output_raw", f"output_second_{i}.txt")
    with open(output_raw_second_path, 'w', encoding='utf-8') as f:
